...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("banccoin"))
        return false;
...
{
    // Convert banccoin:// to banccoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("banccoin://"))
    {
        uri.replace(0, 12, "banccoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BancCoin.lnk";
}
...
{
    return GetAutostartDir() / "banccoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BancCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  banccoin-qt [" + tr("command-line options") + "]                     " + "\n";
